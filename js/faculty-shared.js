...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function storedFacultyPopupIds() {
    try {
      const value = JSON.parse(localStorage.getItem("profConsultPopupShown:faculty") || "[]");
      return Array.isArray(value) ? value.map(String) : [];
    } catch (error) {
      return [];
    }
  ...
  function saveFacultyPopupIds(ids) {
    localStorage.setItem("profConsultPopupShown:faculty", JSON.stringify(ids.slice(-80)));
  }
  ...
  function ensureFacultyNotificationPopupStyles() {
  ...
  function showFacultyInAppNotificationPopup(notification) {
    ensureFacultyNotificationPopupStyles();
  ...
  function showUnreadFacultyNotificationPopups(notifications) {
    ...
    const freshUnread = (notifications || []).filter((notification) => {
      const id = facultyNotificationId(notification);
      return id && notification.Read_Status === "unread" && !knownIds.has(id);
    });

    if (freshUnread.length === 0) return;

    showFacultyInAppNotificationPopup(freshUnread[0]);
  ...
  async function refreshFacultyNotificationBellStatus() {
    ...
    try {
      ...
      if (hasUnread) {
        showUnreadFacultyNotificationPopups(result.notifications || []);
